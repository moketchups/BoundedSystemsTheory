...
# Auto-push to GitHub for transparency/no manipulation proof
AUTO_PUSH = True


def git_push_results(filename: str, model_name: str):
    """Commit and push results to GitHub immediately for transparency."""
    if not AUTO_PUSH:
        return
    ...
    try:
        # Get the repo root (moketchups_engine is inside demerzel repo)
        repo_root = BASE_DIR.parent

        # Stage the new results file
        rel_path = f"moketchups_engine/probe_runs/{Path(filename).name}"
        subprocess.run(
            ["git", "add", rel_path],
            cwd=repo_root,
            capture_output=True,
        ...
        # Commit with timestamp and model name
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        commit_msg = f"[AUTO] Probe result: {model_name} @ {timestamp}\n\nNo manipulation - pushed immediately after probe completion."
        subprocess.run(
        ...
        if result.returncode == 0:
            print(f"  >> AUTO-PUSHED to GitHub: {rel_path}")
        else:
...
def run_probe(model_key: str, include_q14: bool = True, verbose: bool = True) -> dict:
    ...
    # Auto-push to GitHub for transparency
    git_push_results(filename, model_name)
...
def run_all_models(include_q14: bool = True):
    ...
    # Auto-push combined results
    git_push_results(filename, "ALL_MODELS")
