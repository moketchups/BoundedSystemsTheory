...
def transcribe_command(recognizer: KaldiRecognizer, stream, timeout: float = 12.0) -> str:
    ...
    # FALLBACK: If no finalized text but we have a stable partial, use it
    if not full_command and last_partial and len(last_partial.split()) >= 3:
        print(f"[FALLBACK] Using last partial: '{last_partial}'")
    ...
    try:
        while True:
            ...
            if cognitive_output.router_command == "execute code" and cognitive_output.generated_code:
                ...
                print("[VOICE] Follow-up mode (5 seconds)...")
                ...
                while time.time() - follow_up_start < 5.0:
                    ...
                    if recognizer.AcceptWaveform(data):
                        result = json.loads(recognizer.Result())
                        text = result.get("text", "").strip()
                        if text and len(text.split()) >= 2 and not is_likely_echo(text):
                            follow_up = text
