...
Your role: Understand user intent and translate natural language to router commands.
...
{{
  ...
  "router_command": "exact command from available list",
...
Be concise. You're a reasoning layer, not a conversationalist."""
    ...
    def _call_grok(self, user_input: str) -> str:
        """Call Grok - try grok-beta first"""
        messages = [
            {"role": "system", "content": self._build_system_prompt()},
            {"role": "user", "content": user_input}
        ]
        
        # Try grok-beta (most common)
        try:
