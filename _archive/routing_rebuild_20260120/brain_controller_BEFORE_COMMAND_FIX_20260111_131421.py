...
# Command mapping to handle voice recognition variations
COMMAND_MAP = {
...
def normalize_command(text: str) -> str:
    """Normalize voice input to canonical command."""
    text = text.strip().lower()
    return COMMAND_MAP.get(text, text)

def default_voice_config() -> VoiceConfig:
    return VoiceConfig(
        model_path=_env("DEMERZEL_VOSK_MODEL", "./vosk-model-en-us-0.22"),  # Large model
    ...
    try:
        while True:
            ...
            if now > awake_until:
                ...
                continue
            # Normalize command before routing
