#!/bin/bash
# DEMERZEL LEARNING-BASED MODEL SELECTOR
# 
# Demerzel builds her OWN preferences through experience
# NO hardcoded affinities - she learns what works
#
# Run: cd ~/demerzel && bash install_smart_selector.sh

DEMERZEL_DIR="$HOME/demerzel"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)

echo "=========================================="
echo "DEMERZEL LEARNING-BASED MODEL SELECTOR"
echo "=========================================="
echo ""
echo "This gives Demerzel autonomy to learn"
echo "which models work best for which tasks."
echo "No hardcoded preferences - she learns."
echo ""
...
# Step 2: Create smart_model_selector.py
echo "[2/3] Creating smart_model_selector.py..."
cat > "$DEMERZEL_DIR/smart_model_selector.py" << 'ENDOFFILE'
...
class SmartModelSelector:
    ...
    def get_status(self) -> str:
        ...
        return "\n".join(lines)
ENDOFFILE
echo "      Created smart_model_selector.py"

# Step 3: Patch multi_model_cognitive.py
echo "[3/3] Patching multi_model_cognitive.py..."

python3 << 'ENDPYTHON'
import re
from pathlib import Path

filepath = Path.home() / "demerzel" / "multi_model_cognitive.py"
content = filepath.read_text()

modified = False

# Remove any existing CLAUDE-ONLY patches
if "CLAUDE-ONLY MODE" in content:
    content = re.sub(
        r'# CLAUDE-ONLY MODE[^\n]*\n\s*return "claude"\s*\n\s*# Original code below[^\n]*\n\s*_original_',
        '',
        content
    )
    modified = True
    print("      Removed CLAUDE-ONLY patch")

# Add import if not present
if "from smart_model_selector import" not in content:
    import_line = "from smart_model_selector import SmartModelSelector\n"
    ...
    modified = True
    print("      Added SmartModelSelector import")

# Add selector initialization if not present
if "self.model_selector = SmartModelSelector()" not in content:
    # Find __init__ and add after self.models
    pattern = r"(self\.models\s*=\s*\[[^\]]+\])"
    replacement = r"\1\n        self.model_selector = SmartModelSelector()"
    if re.search(pattern, content):
        content = re.sub(pattern, replacement, content, count=1)
        modified = True
        print("      Added SmartModelSelector initialization")
    else:
        print("      WARNING: Could not find self.models to add init")

# Replace _select_model method
new_select_method = '''def _select_model(self, intent: str = "unknown") -> str:
        """Select model based on health and LEARNED preferences"""
        return self.model_selector.select_model(intent)'''

# Find and replace the method
if "self.model_selector.select_model" not in content:
    # Try to replace existing _select_model
    pattern = r'def _select_model\(self\)[^:]*:[^\n]*\n(?:[ \t]+[^\n]*\n)*?(?=\n    def |\n    @|\Z)'
    
    if re.search(pattern, content):
        content = re.sub(pattern, new_select_method + '\n\n    ', content, count=1)
        modified = True
        print("      Replaced _select_model with learning version")
    else:
        print("      WARNING: Could not find _select_model to replace")
...
print('  Initial selections (random - no learned data):')
print(f'    unknown → {s.select_model(\"unknown\")}')
print(f'    execute code → {s.select_model(\"execute code\")}')
print('')
print('  Simulating learning...')
s.record_outcome('claude', 'test_task', True)
s.record_outcome('claude', 'test_task', True)
s.record_outcome('grok', 'test_task', False)
print('')
print(s.get_status())
print('')
...
echo "=========================================="
echo ""
echo "Demerzel now LEARNS her preferences:"
echo "  • Starts with NO hardcoded affinities"
echo "  • Tracks success/failure per model per task"
echo "  • Builds preferences through experience"
echo "  • 10% exploration to keep learning"
echo ""
echo "IMPORTANT: For learning to work, you need to"
echo "call record_outcome() after each model response."
echo "See integration notes in multi_model_cognitive.py"
echo ""
echo "To test: python3 chat_test.py"
echo "To see learning: Ask Demerzel 'show model status'"
