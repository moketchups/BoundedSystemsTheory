...
class MultiModelCognitive:
    ...
    def _get_next_model(self, force_model: Optional[str] = None) -> str:
        """
        Get next model for processing.
        
        DEMERZEL'S FIX: Skip models that have failed repeatedly.
        ...
        while attempts < len(self.models):
            ...
            if self.model_failures.get(model, 0) >= self.max_failures_before_skip:
                print(f"[MODEL] Skipping {model} (too many failures)")
                attempts += 1
                continue
...
"""
    ...
    def process(self, user_input: str, transcript_confidence: float = 1.0) -> CognitiveOutput:
        ...
        # Select model
        model = self._get_next_model(force_model=force_model)
