    ...
    while time.time() - start_time < timeout and not shutdown_requested:
        ...
        if recognizer.AcceptWaveform(data):
            ...
            if partial_text and len(partial_text.split()) >= 1:
                # Skip partials that start with echo words
                first_words = partial_text.lower().split()[:2]
                echo_starters = ['result', 'resolved', 'resolve', 'code', 'executed', 'error']
                if any(w in echo_starters for w in first_words):
                    continue
